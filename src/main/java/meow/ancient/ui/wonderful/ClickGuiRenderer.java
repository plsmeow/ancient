package meow.ancient.ui.wonderful;
...
import meow.ancient.util.render.math.Scissor;
...
public class ClickGuiRenderer {
    ...
    private void renderSidebar(int mouseX, int mouseY, float x, float y, int accent, float alpha) {
        ...
        String selfName = ClickGuiLayout.truncate(
                meow.ancient.util.IMinecraft.mc.getSession().getUsername(), 7f, innerW - 12f);
    ...
    private void drawPanelBase(float x, float y, float w, float h, float alpha) {
        if (meow.ancient.module.list.render.Optimization.shouldDisableClickGuiBlur()) {
        ...
        DrawUtil.drawRound(x, y, w, h, ClickGuiLayout.RADIUS,
                ColorProvider.rgba(15, 12, 21, (int) ((meow.ancient.module.list.render.Optimization.shouldDisableClickGuiBlur() ? 245 : 225) * alpha)));
    }
}
