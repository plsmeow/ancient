package meow.ancient.module.list.render.hud.celestial;
    ...
    public static void renderCelestial(Interface hud, DrawContext context) {
        ...
        if (playerEntity != null) {
            ...
            if (spawnEgg != null) {
                ...
                DrawUtil.drawText(meow.ancient.util.render.msdf.Fonts.ICONS_NURIK.get(), "N", headX + 1.5f, headY + 8f, ColorProvider.rgba(255, 255, 255, alphaInt), 24f);
            }
        }

        float textX = headX + headSize + 2;
        String name = livingEntity.getName().getString();
        DrawUtil.drawText(meow.ancient.util.render.msdf.Fonts.SFBOLD.get(), name, textX, y + 5, ColorProvider.rgba(230, 230, 230, alphaInt), 9f, 0.3f, 0.7f, width);
        ...
        DrawUtil.drawText(meow.ancient.util.render.msdf.Fonts.SFBOLD.get(), hpText, textX + 0.5F, hpY, ColorProvider.rgba(230, 230, 230, alphaInt), fontSize);

        if (absHp > 0.05f) {
            String absText = String.format(java.util.Locale.US, "%.1f AB", absHp);
            float hpW = meow.ancient.util.render.msdf.Fonts.SFBOLD.get().getWidth(hpText, fontSize), plusW = meow.ancient.util.render.msdf.Fonts.SFBOLD.get().getWidth("  + ", fontSize);
            DrawUtil.drawText(meow.ancient.util.render.msdf.Fonts.SFBOLD.get(), "  + ", textX + hpW, hpY, ColorProvider.rgba(160, 160, 160, alphaInt), fontSize);
            DrawUtil.drawText(meow.ancient.util.render.msdf.Fonts.SFBOLD.get(), absText, textX + hpW + plusW, hpY, ColorProvider.rgba(255, 205, 70, alphaInt), fontSize);
        }
