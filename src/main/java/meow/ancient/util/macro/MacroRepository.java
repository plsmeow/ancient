package meow.ancient.util.macro;
...
public class MacroRepository implements IMinecraft, QuickLogger {
    ...
    private final File file = new File(".options/macros.json");
    ...
    public void onKey(EventKeyInput event) {
        ...
        for (Macro macro : macroList) {
            ...
            String commandPrefix = Ancient.getInstance().getCommandDispatcher().getCommandPrefix();
            if (msg.startsWith(commandPrefix)) {
                msg = msg.substring(commandPrefix.length());
                Ancient.getInstance().getCommandDispatcher().runCommand(msg);
            } else {
