...
import meow.ancient.module.list.render.BlockEsp;
...
public class BlockEspCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        BlockEsp module = Ancient.getInstance().getModuleStorage().get(BlockEsp.class);
        if (module == null) {
    ...
    private void printHelp() {
        logDirect("§7Блоки: §f" + Ancient.getInstance().getModuleStorage().get(BlockEsp.class).getTargetBlocks().size());
    ...
    @Override
    public Stream<String> tabComplete(String label, IArgConsumer args) throws meow.ancient.util.commands.api.exception.CommandNotEnoughArgumentsException {
        BlockEsp module = Ancient.getInstance().getModuleStorage().get(BlockEsp.class);
